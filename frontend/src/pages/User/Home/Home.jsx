...
const Home = () => {
  ...
  const fetchAllBooks = async () => {
    const res = await jwtReq.current.get(`/books/list`);
  ...
  const searchBook = async (searchValue) => {
    if (searchValue) {
      const res = await jwtReq.current.get(
        `books/list_filter?name=${searchValue}`
      );
      const books = res.data.data.books;
        ...
        {books?.map((b) => (
          <Card sx={{ width: 345 }}>
            ...
            <CardContent>
              ...
              <Typography variant="body2" color="text.secondary">
                Issued Date: {new Date(b.issued_date).toDateString()}
              </Typography>
            </CardContent>
            <CardActions>
              ...
              <Button
                size="small"
                onClick={() =>
                  navigate(`/customer/book-borrow/${b.id}`)
                }
              >
                Borrow
              </Button>

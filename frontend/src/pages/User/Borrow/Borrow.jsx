...
const Borrow = () => {
  ...
  useEffect(() => {
    if (book) fetchCover(book).then((data) => setImage(data));
  }, [book]);
  ...
  const borrowBook = async () => {
    try {
      await axiosJWT.post(`/book_orders/order_book`, {
    ...
    <div className="borrow">
      ...
      <div className="borrowWrapper">
        ...
        <div className="right">
          ...
          <h4>Description: {book?.desc}</h4>
          <h4>Issued Date: {book?.desc}</h4>

          <div className="borrowBtn">
            {book?.borrow_status ? (
              <h4 style={{ width: 300, padding: 10 }} className="errorBudget">
                Unavailable (Borrowed By other person)
              </h4>
            ) : (
              <ContainedButton
                title={"Borrow Book"}

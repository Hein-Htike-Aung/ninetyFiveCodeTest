...
import "./book_order.scss";
import fetchBookCover from "../../../utils/fetchBookCover";
import useJWTCustomer from "../../../hooks/useJwtCustomer";
...
const BookOrder = () => {
  ...
  const axiosJwt = useJWTCustomer();
  const jwtReq = useRef(axiosJwt);

  useEffect(() => {
    fetchAllCustomerBooks(currentPage);
  }, [currentPage]);

  const fetchAllCustomerBooks = async (currentPage) => {
    if (customer) {
      const res = await jwtReq.current.get(
        `/book_orders/for_customer?customerID=${customer.id}&page=${currentPage}&pageSize=10`
      );
      ...
      if (books?.length) {
        await Promise.all(
          books.map(async (b) => {
            if (b.Book.cover_photo) {
              await fetchBookCover(b.Book);
            }
          })
        );
      }
      setBooks(books);
      setBooksCount(Math.ceil(res.data.data.count / 10));
    }
  };

  console.log(books);

  const returnBook = async (bookOrderId) => {
    const res = await jwtReq.current.post(
      `/book_orders/return_book/${bookOrderId}`
    );

    if (res.status === 200) {
      toast.success("Book has returned");
      fetchAllCustomerBooks(1);
      setCurrentPage(1);
    }
  };
          ...
          <TableBody>
            {books?.map((row, idx) => (
              ...
              >
                ...
                <TableCell>{row.Book.category}</TableCell>
                <TableCell>

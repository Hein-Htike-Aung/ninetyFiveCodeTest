...
const LibraryBooks = () => {
  ...
  const fetchAllBooks = async (currentPage) => {
    setFetching(true);
    const res = await jwtReq.current.get(
      `/books/list?page=${currentPage}&pageSize=10`
    );
  ...
  const searchBook = async (searchValue) => {
    if (searchValue) {
      const res = await jwtReq.current.get(
        `books/list_filter?name=${searchValue}&page=${currentPage}&pageSize=10`
      );
      const books = res.data.data.books;
  ...
  const confirmDelete = async () => {
    if (targetBook) {
      const res = await jwtReq.current.delete(`/books/delete/${targetBook.id}`);
  ...
  return (
    <div className="libraryBooks">
      <ContentTitle title="Books">
        <OutlinedButton
          btnClick={() => navigate("/librarian-user/books/add-book/0")}
        >
          <AddIcon /> Add Book
        </OutlinedButton>
      </ContentTitle>
              ...
              <TableBody>
                {books?.map((row, idx) => (
                  ...
                  >
                    ...
                    <TableCell>
                      {new Date(row.issued_date).toDateString()}
                    </TableCell>
                    <TableCell>
                      {row.borrow_status ? (
                        <span className="errorBudget">Borrowed</span>
                      ) : (
                        <span className="successBudget">Available</span>
                      )}
                    </TableCell>
                    <TableCell align="right">
                      <div className="tableIconsWrapper">
                        <div
                          className="tableIcon"
                          onClick={() =>
                            navigate(`/librarian-user/books/add-book/${row.id}`)
                          }
                        >
                          <CreateIcon />
                        </div>
